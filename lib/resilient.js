...
Resilient.prototype.request = function (path, options) {
  return new Promise( function(resolve, reject) {
      this._client.send(path, options, function(err, res) {
        if (err) {
          reject(err);
        } else {
          resolve(res);
...
;['get', 'post', 'put', 'del', 'delete', 'head', 'patch']
  .forEach(function defineMethodProxy (verb) {
    Resilient.prototype[verb] = function (path, options) {
      return new Promise(function(resolve, reject) {
          this._client[verb](path, options, function(err, res) {
              if (err) {
                  reject(err);
              } else {
                  resolve(res);
