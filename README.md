...
```html
<script src="//cdn.rawgit.com/resilient-http/resilient.js/0.1.6/resilient.js"></script>
```
...
- **promiscuousErrors** `boolean` - Enable promiscuous error handling mode. Client HTTP status errors (400-499) will be handled as failed requests and Resilient will not resolve it, retrying the request until it has valid status (if configured). Default `false`
...
- **promiscuousErrors** `boolean` - Enable promiscuous error handling mode. Client HTTP status errors (400-499) will be handled as failed requests and Resilient will not resolve it, retrying the request until it has valid status (if configured). Default `true`
